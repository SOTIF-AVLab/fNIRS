...
# pdb.set_trace()
if os.path.isfile('Table1_Result.xlsx'):
    os.remove('Table1_Result.xlsx')
else:
    print('There are no Table1_Result.xlsx')
#####################################
MAX_ITER = 100
TOLERANCE = 1e-6
NUM_CHANNEL=8
...
Risk_Mean_Value=np.zeros([3,Num_Event])
num_i_event=0
writer_Risk_Mean_Value = pd.ExcelWriter('Table1_Result.xlsx')  #创建名称为hhh的excel表格
for i_event in All_Event_Type:#####load each event data
    ...
    for i_nub_sample in range (i_all):
        ...
        Risk_Value[i_nub_sample,0]=np.mean((TH_Low_DATA    [i_nub_sample,MMTH_Column_Risk,:]))
        Risk_Value[i_nub_sample,1]=np.mean((TH_Hight_DATA  [i_nub_sample,MMTH_Column_Risk,:]))
    ...
    # ###################################save date to excel############################
    Risk_Value_df  =pd.DataFrame(Risk_Value)#将ndarray格式转换为DataFrame
    Risk_Value_df.to_excel (writer_Risk_Mean_Value,sheet_name=i_event+'THRisk')
 # ###################################save date to excel############################
Risk_Mean_Value_df  =pd.DataFrame(Risk_Mean_Value)#将ndarray格式转换为DataFrame
Risk_Mean_Value_df.to_excel (writer_Risk_Mean_Value,sheet_name='mean_value')
writer_Risk_Mean_Value.save()